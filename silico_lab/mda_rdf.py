...
# Target: protein heavy atoms except carbons (and hydrogens)
TARGET_SELECTION = "(protein or resname NAG BMA MAN AMA BGC FUC AFUC SIA NANA NGA GAL BGAL AGAL GLC AGLC BGLC) and not name C* and not name H*"
...
for r_idx, xtc in enumerate(args.xtc):
    ...
    contact_counts = [np.zeros(int(m.sum()), dtype=int) for m in ref_masks]
    sum_volume = 0.0
    ...
    for i, ts in enumerate(traj_slice):
        ...
        for k in range(len(ref_idx)):
            ...
            hists[k] += counts
            contact_counts[k] += (dists < args.cutoff).astype(int)
    ...
    for k, (sel, res_label, bs_label) in enumerate(valid_atoms):
        ...
        contact_data[(rep_label, res_label)] = (contact_counts[k], n_frames)
...
fig = make_subplots(
    ...
    row_titles=unique_bs, column_titles=unique_replicas,
)
...
for k, (sel, res_label, bs_label) in enumerate(valid_atoms):
    ...
    for rep in unique_replicas:
        if (rep, res_label) in contact_data:
            counts, n_frames = contact_data[(rep, res_label)]
...
for k, (sel, res_label, bs_label) in enumerate(valid_atoms):
    ...
    for c_idx, rep in enumerate(unique_replicas, start=1):
        if (rep, res_label) not in contact_data:
            continue
        counts, n_frames = contact_data[(rep, res_label)]
        freq = counts[sel_local] / n_frames
        fig_contacts.add_trace(
            go.Bar(
                x=target_labels, y=freq,
                marker=dict(color=color),
                showlegend=False,
                hovertemplate=(f"{res_label} ({rep})<br>"
                               f"partner: %{{x}}<br>"
                               f"frequency = %{{y:.3f}}<extra></extra>"),
            ),
            row=row, col=c_idx,
        )
...
fig_contacts.update_layout(
    ...
    width=max(900, 450 * n_cols_c) + 280,
    template="plotly_white",
    title=f"Contact frequencies within {args.cutoff} Å",
)
